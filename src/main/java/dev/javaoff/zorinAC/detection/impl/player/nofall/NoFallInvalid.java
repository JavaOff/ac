...
public class NoFallInvalid implements Detection {
    ...
    public void register() {
        PacketEvents.getAPI().getEventManager().registerListener(new PacketListenerAbstract(PacketListenerPriority.HIGHEST) {
            ...
            public void onPacketReceive(PacketReceiveEvent event) {
                ...
                if (currentFallDistance >= FALL_DISTANCE_THRESHOLD && Math.abs(currentLoc.getY()) > INVALID_Y_THRESHOLD) {
                    bufferValue += 1.0;
                    if (bufferValue > BUFFER_LIMIT) {
                        ZorinAC.flagManager().flag(player, "NoFallInvalid");
                    }
                }
