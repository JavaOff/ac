...
public class NoFallMicro implements Detection {
    ...
    public void register() {
        PacketEvents.getAPI().getEventManager().registerListener(new PacketListenerAbstract(PacketListenerPriority.HIGHEST) {
            ...
            public void onPacketReceive(PacketReceiveEvent event) {
                ...
                if (currentFallDistance >= FALL_DISTANCE_THRESHOLD && flying.hasPositionChanged() &&
                        Math.abs(currentLoc.getY() - previousLoc.getY()) < MICRO_Y_THRESHOLD &&
                        Math.abs(currentLoc.getY() - previousLoc.getY()) > 0) {
                    bufferValue += 1.0;
                    if (bufferValue > BUFFER_LIMIT) {
                        ZorinAC.flagManager().flag(player, "NoFallMicro");
                    }
                }
